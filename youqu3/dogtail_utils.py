...
import easyprocess
from youqu3.custom_exception import ElementNotFound
from youqu3.custom_exception import ApplicationStartError

from youqu3.vendor.dogtail.tree import SearchError
from youqu3.vendor.dogtail.tree import root
from youqu3.vendor.dogtail.tree import predicate
from youqu3.vendor.dogtail.tree import config
from youqu3.vendor.dogtail.tree import Node
from youqu3 import MouseKey
...
class DogtailUtils(MouseKey):
    ...
    def __init__(self, name=None, description=None, number=-1, check_start=True, key: dict = None):
        ...
        try:
            ...
            if check_start:
                search_app = easyprocess.EasyProcessError(easyprocess.split_command(f"ps -ef | grep {self.name}"))
...
if __name__ == '__main__':
    dog = DogtailUtils().element_click("Btn_文件管理器")