...
from youqu3.vendor.dogtail.config import config
from youqu3.vendor.dogtail import path
from youqu3.vendor.dogtail import predicate
from youqu3.vendor.dogtail import rawinput
from youqu3.vendor.dogtail.logging import debugLogger as logger
from youqu3.vendor.dogtail.utils import doDelay, Blinker, Lock
...
gi = importlib.import_module(f"youqu3.vendor.gi_{setting.ARCH}")
Gdk = importlib.import_module(f"youqu3.vendor.gi_{setting.ARCH}.repository")
import os
import sys
...
if config.checkForA11y:
    from youqu3.vendor.dogtail.utils import checkForA11y
    checkForA11y()
...
class Node(object):
    ...
    def dump(self, type='plain', fileName=None):
        from youqu3.vendor.dogtail import dump
    ...
    def _fastFindChild(self, pred, recursive=True, showingOnly=None):
        """
        Searches for an Accessible using methods from youqu3.vendor.pyatspi.utils
        """
